...
export function validateAssetImport(action){
  ...
  return{type:'asset-import',items:action.items.map((item,index)=>{const id=String(item.id||''),symbol=String(item.symbol||'').trim().toUpperCase(),name=String(item.name||'').trim();if(!/^[a-zA-Z0-9-]{1,64}$/.test(id)||!/^[^\s]{1,7}$/.test(symbol)||!name||[...name].length>30)throw new Error(`Revise a sigla e o nome do ativo ${index+1}.`);if(!Number.isInteger(item.assetType)||![[1,1],[1,2],[2,4],[2,5]].some(pair=>pair[0]===item.assetType&&pair[1]===item.subType))throw new Error(`Classificação inválida no ativo ${index+1}.`);if(!Number.isInteger(item.quantity)||item.quantity<0||item.quantity>2147483647)throw new Error(`Quantidade inválida no ativo ${index+1}.`);for(const key of ['currentPrice','total']){const max=key==='currentPrice'?999999.99:99999999.99;if(typeof item[key]!=='number'||!Number.isFinite(item[key])||item[key]<0||item[key]>max||Math.abs(item[key]*100-Math.round(item[key]*100))>0.000001)throw new Error(`Valor inválido no ativo ${index+1}.`);}if(item.quantity===0&&item.total!==0)throw new Error(`Quantidade incompatível com o valor total no ativo ${index+1}.`);if(item.quantity&&item.total/item.quantity>999999.99)throw new Error(`Preço médio calculado acima do limite no ativo ${index+1}.`);return{...item,id,symbol,name};})};
}
