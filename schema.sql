...
CREATE TABLE IF NOT EXISTS monthly_expense_entries (expense_id TEXT NOT NULL,month INTEGER NOT NULL CHECK(typeof(month)='integer' AND month BETWEEN 1 AND 12),year INTEGER NOT NULL CHECK(typeof(year)='integer' AND year BETWEEN 1900 AND 9999),created_at TEXT NOT NULL DEFAULT(strftime('%Y-%m-%dT%H:%M:%fZ','now')),PRIMARY KEY(expense_id,month,year),FOREIGN KEY(expense_id) REFERENCES monthly_expenses(id) ON DELETE CASCADE);
