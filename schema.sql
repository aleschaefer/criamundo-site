...
CREATE TABLE IF NOT EXISTS finance_transactions (
  ...
  updated_at TEXT,
  revision INTEGER NOT NULL DEFAULT 0,
  FOREIGN KEY (asset_id, owner, name, type, subtype) REFERENCES finance_assets(id, owner, name, type, subtype) ON UPDATE CASCADE ON DELETE RESTRICT
);
...
END;
CREATE TRIGGER IF NOT EXISTS finance_asset_fixed_dates_insert BEFORE INSERT ON finance_assets
...
CREATE TRIGGER IF NOT EXISTS finance_asset_fixed_dates_update BEFORE UPDATE OF type, entry_date, exit_date ON finance_assets
WHEN (NEW.entry_date IS NOT NULL OR NEW.exit_date IS NOT NULL) AND
  (NEW.type != 2 OR (NEW.exit_date IS NOT NULL AND (NEW.entry_date IS NULL OR NEW.exit_date < NEW.entry_date)))
BEGIN SELECT RAISE(ABORT, 'Datas de renda fixa inválidas.'); END;
...
-- Autenticação administrativa com senha e passkey/WebAuthn.
CREATE TABLE IF NOT EXISTS admin_users (id TEXT PRIMARY KEY NOT NULL,email TEXT NOT NULL UNIQUE COLLATE NOCASE,password_salt TEXT NOT NULL,password_hash TEXT NOT NULL,password_iterations INTEGER NOT NULL DEFAULT 100000,created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),updated_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')));
CREATE TABLE IF NOT EXISTS admin_passkeys (credential_id TEXT PRIMARY KEY NOT NULL,user_id TEXT NOT NULL,public_key TEXT NOT NULL,sign_count INTEGER NOT NULL DEFAULT 0,transports TEXT,created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),last_used_at TEXT,FOREIGN KEY(user_id) REFERENCES admin_users(id) ON DELETE CASCADE);
CREATE TABLE IF NOT EXISTS admin_auth_challenges (id TEXT PRIMARY KEY NOT NULL,user_id TEXT,purpose TEXT NOT NULL CHECK(purpose IN ('setup','login')),challenge TEXT NOT NULL,payload TEXT,expires_at TEXT NOT NULL,created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),FOREIGN KEY(user_id) REFERENCES admin_users(id) ON DELETE CASCADE);
CREATE TABLE IF NOT EXISTS admin_sessions (token_hash TEXT PRIMARY KEY NOT NULL,user_id TEXT NOT NULL,expires_at TEXT NOT NULL,created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),last_seen_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),FOREIGN KEY(user_id) REFERENCES admin_users(id) ON DELETE CASCADE);
...
-- Gastos mensais recorrentes e seleção por competência.
CREATE TABLE IF NOT EXISTS monthly_expense_groups (id TEXT PRIMARY KEY NOT NULL,name CHAR(30) COLLATE NOCASE NOT NULL UNIQUE CHECK(length(trim(name)) BETWEEN 1 AND 30 AND name=trim(name)),revision INTEGER NOT NULL DEFAULT 0,created_at TEXT NOT NULL DEFAULT(strftime('%Y-%m-%dT%H:%M:%fZ','now')));
CREATE TABLE IF NOT EXISTS monthly_expenses (id TEXT PRIMARY KEY NOT NULL,owner TEXT NOT NULL CHECK(owner IN('Ale','Ana')),name CHAR(50) NOT NULL CHECK(length(trim(name)) BETWEEN 1 AND 50 AND name=trim(name)),value DECIMAL(10,2) NOT NULL CHECK(value>=0 AND value<=99999999.99 AND value=round(value,2)),group_id TEXT NOT NULL,payment_date TEXT CHECK(payment_date IS NULL OR payment_date=date(payment_date,'+0 days')),payment_day INTEGER CHECK(payment_day IS NULL OR (typeof(payment_day)='integer' AND payment_day BETWEEN 1 AND 31)),settled INTEGER NOT NULL DEFAULT 0 CHECK(settled IN(0,1)),revision INTEGER NOT NULL DEFAULT 0,created_at TEXT NOT NULL DEFAULT(strftime('%Y-%m-%dT%H:%M:%fZ','now')),FOREIGN KEY(group_id) REFERENCES monthly_expense_groups(id) ON UPDATE CASCADE ON DELETE RESTRICT);
CREATE TABLE IF NOT EXISTS monthly_expense_entries (expense_id TEXT NOT NULL,month INTEGER NOT NULL CHECK(typeof(month)='integer' AND month BETWEEN 1 AND 12),year INTEGER NOT NULL CHECK(typeof(year)='integer' AND year BETWEEN 1900 AND 9999),settled INTEGER NOT NULL DEFAULT 0 CHECK(settled IN(0,1)),disregarded INTEGER NOT NULL DEFAULT 0 CHECK(disregarded IN(0,1)),created_at TEXT NOT NULL DEFAULT(strftime('%Y-%m-%dT%H:%M:%fZ','now')),PRIMARY KEY(expense_id,month,year),FOREIGN KEY(expense_id) REFERENCES monthly_expenses(id) ON DELETE CASCADE);
CREATE INDEX IF NOT EXISTS idx_monthly_expense_entries_period ON monthly_expense_entries(year,month);
CREATE TABLE IF NOT EXISTS monthly_incomes (id TEXT PRIMARY KEY NOT NULL,owner TEXT NOT NULL CHECK(owner IN('Ale','Ana')),name CHAR(50) NOT NULL CHECK(length(trim(name)) BETWEEN 1 AND 50 AND name=trim(name)),value DECIMAL(10,2) NOT NULL CHECK(value>=0 AND value<=99999999.99 AND value=round(value,2)),month INTEGER CHECK(month IS NULL OR(typeof(month)='integer' AND month BETWEEN 1 AND 12)),year INTEGER CHECK(year IS NULL OR(typeof(year)='integer' AND year BETWEEN 1900 AND 9999)),revision INTEGER NOT NULL DEFAULT 0,created_at TEXT NOT NULL DEFAULT(strftime('%Y-%m-%dT%H:%M:%fZ','now')));
