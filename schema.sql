...
CREATE TABLE IF NOT EXISTS monthly_expenses (id TEXT PRIMARY KEY NOT NULL,owner TEXT NOT NULL CHECK(owner IN('Ale','Ana')),name CHAR(50) NOT NULL CHECK(length(trim(name)) BETWEEN 1 AND 50 AND name=trim(name)),value DECIMAL(10,2) NOT NULL CHECK(value>=0 AND value<=99999999.99 AND value=round(value,2)),group_id TEXT NOT NULL,payment_date TEXT CHECK(payment_date IS NULL OR payment_date=date(payment_date,'+0 days')),settled INTEGER NOT NULL DEFAULT 0 CHECK(settled IN(0,1)),revision INTEGER NOT NULL DEFAULT 0,created_at TEXT NOT NULL DEFAULT(strftime('%Y-%m-%dT%H:%M:%fZ','now')),FOREIGN KEY(group_id) REFERENCES monthly_expense_groups(id) ON UPDATE CASCADE ON DELETE RESTRICT);
