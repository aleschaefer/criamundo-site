...
async function overview(db){const [groups,expenses,entries]=await db.batch([
...
]);return{groups:groups.results,expenses:expenses.results,entries:entries.results};}
export async function handleMonthlyExpenses(request,env){
  ...
  try{
    ...
    console.error('Monthly expenses database error',error);return reply({error:'Não foi possível acessar Gastos Mensais. Aplique a migração 0024.'},503);
  }
}
