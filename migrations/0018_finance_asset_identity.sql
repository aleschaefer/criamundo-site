-- Aplicar uma única vez após 0017. Inclui a sigla na identidade única do ativo.
-- Preserva IDs, saldos, classificações, datas, revisões e histórico.
...
INSERT INTO finance_assets_new (id, name, symbol, type, subtype, quantity, average_price, value, current_price, current_dy, revision, created_at, updated_at, current_income)
SELECT id, name, symbol, type, subtype, quantity, average_price, value, current_price, current_dy, revision, created_at, updated_at, current_income FROM finance_assets ORDER BY rowid;
...
SELECT id, asset_id, name, type, subtype, quantity, value, created_at, updated_at, transaction_date, revision FROM finance_transactions ORDER BY rowid;
