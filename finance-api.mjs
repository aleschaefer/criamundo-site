...
async function overview(db) {
  ...
  const [assets, transactions] = await db.batch([
    db.prepare('SELECT id, name, type AS assetType, quantity, average_price AS averagePrice, value AS total, COALESCE(current_price, average_price) AS currentPrice, current_dy AS currentDy FROM finance_assets ORDER BY name, type'),
...
export async function handleFinance(request, env) {
  ...
  try {
    ...
    if (action.type === 'asset') {
      await db.prepare(`INSERT INTO finance_assets (id, name, type, quantity, average_price, value, current_price, current_dy)
        VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8) ON CONFLICT(id) DO NOTHING`)
        .bind(action.id, action.name, action.assetType, action.quantity, action.averagePrice, action.value, action.currentPrice, action.currentDy).run();
    } else {
    ...
    return reply({ error: 'Não foi possível acessar Finanças. Verifique a conexão e verifique se as migrações 0001 e 0002 foram aplicadas no banco.' }, 503);
  }
}
