...
async function overview(db) {
  ...
  const [assets, transactions] = await db.batch([
    db.prepare('SELECT id, name, type AS assetType, quantity, average_price AS averagePrice, value AS total, COALESCE(current_price, average_price) AS currentPrice, current_income AS currentIncome FROM finance_assets ORDER BY name, type'),
...
export async function handleFinance(request, env) {
  ...
  try {
    ...
    return reply({ error: 'Não foi possível acessar Finanças. Verifique a conexão e se as migrações 0001, 0002 e 0003 foram aplicadas no banco.' }, 503);
  }
}
